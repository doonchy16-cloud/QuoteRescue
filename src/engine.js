...
export function generateRecoveryPlan(raw = {}) {
  ...
  let nextMove = diagnosisResult.nextMove;
  if (input.lastContactAgeDays === 0 && context.recoveryMode !== 'blocked') {
    nextMove = `Do not send another follow-up today. Start on the next appropriate business day. ${nextMove}`;
  }
