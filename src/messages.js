...
const TONE = Object.freeze({
  warm: {
    greeting:(n)=>`Hi ${n} — hope you’re doing well.`,
    ask:'Would it be helpful if I made the next step easier?',
    close:'No pressure at all—just let me know what works best for you.'
  },
  concise: {
    greeting:(n)=>`Hi ${n} — quick follow-up.`,
    ask:'Want me to clarify the one thing holding this up?',
    close:'A quick yes, later, or close it out is perfect.'
  },
  consultative: {
    greeting:(n)=>`Hi ${n} — I wanted to follow up and make the decision easier.`,
    ask:'What would be most useful for you to clarify before deciding?',
    close:'I’m happy to help you get to a clean decision either way.'
  },
  premium: {
    greeting:(n)=>`Hi ${n} — I’m following up to make sure the proposal is fully aligned with your project priorities.`,
    ask:'Would you like a concise review of the scope, assumptions, and next decision?',
    close:'I’m happy to keep this organized and easy to evaluate.'
  },
  direct: {
    greeting:(n)=>`Hi ${n} — checking in on the estimate.`,
    ask:'What is the main blocker right now?',
    close:'Should I keep it open, circle back later, or close it out?'
  }
});
...
function identity(input) {
  const business = input.businessName ? ` with ${input.businessName}` : '';
  return `${input.repName}${business}`;
}

function projectRef(input) {
  return shortProjectReference(input.jobDescription, 64) || input.trade;
}

function amountRef(input) {
  return input.quoteAmount ? ` ($${Math.round(input.quoteAmount).toLocaleString()})` : '';
}

function boundedSms(text) {
  const clean = sanitizeSingleLine(text, 1000);
  return clean.length <= 320 ? clean : `${clean.slice(0, 317).trimEnd()}…`;
}

function subjectFor(input) {
  const trade = sanitizeSingleLine(input.trade, 60) || 'project';
  const subject = `Quick follow-up on your ${trade} estimate`;
  return subject.length <= 90 ? subject : `${subject.slice(0, 89)}…`;
}
...
function obstructionCopy(input, context) {
  const t = TONE[input.tone];
  const ref = projectRef(input);
  const guidance = BLOCKER_GUIDANCE[context.primaryBlocker] ?? BLOCKER_GUIDANCE.unknown;
...
function reactivation(input) {
  const ref = projectRef(input);
  const t = TONE[input.tone];
  return boundedSms(`${input.customerName}, I’m revisiting the ${input.trade} estimate for ${ref}. Has the project become relevant again, or is it still on hold? ${t.close}`);
}
...
export function buildDiagnosis(input, context) {
  ...
  if (context.recoveryMode === 'close_loop' || context.recoveryMode === 'reactivation') return { diagnosis:'This is no longer a normal active follow-up. The right move is to close the loop or create a clean reactivation point.', nextMove:'Use one respectful close-loop message, then stop repeated chasing. Reactivate only when the opportunity becomes relevant again.' };
  const blocker = context.primaryBlocker;
  if (blocker === 'budget') return { diagnosis:'Budget friction is the primary blocker. The recovery path is to clarify priorities and scope before discussing any revised option.', nextMove:'Ask which scope elements matter most, then separate must-haves from optional work without changing the offer unless the customer asks.' };
...
export function buildCampaign(input, context) {
  if (context.recoveryMode === 'blocked' || context.contactState === 'do_not_contact') {
    return { sevenDaySteps:[], reactivation:'', objectionResponse:'', closeLoop:'', sms:'', email:{subject:'',body:''}, voicemail:'' };
  }

  const sms = buildSms(input, context, 'open');
  const email = buildEmail(input, context);
  const voicemail = buildVoicemail(input, context);
  let day0;
  ...
  const sevenDaySteps = [
    day0,
    { day:'Day 2', action:'Resolve blocker', purpose:'Address the most likely blocker without defending the quote.', sms:buildSms(input, context, 'blocker') },
    { day:'Day 4', action:'Clarify value / scope', purpose:'Make the decision easier to evaluate.', sms:buildSms(input, context, 'clarify') },
    { day:'Day 7', action:'Close the loop', purpose:'Give the customer an easy yes / later / no decision.', sms:closeLoop(input) }
  ];

  return {
    sevenDaySteps,
    reactivation:reactivation(input),
    objectionResponse:obstructionCopy(input, context),
    closeLoop:closeLoop(input),
    sms,
    email,
    voicemail
  };
