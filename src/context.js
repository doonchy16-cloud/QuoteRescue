...
const CUES = [
  ...
  { kind:'declined', blocker:null, engagement:'lost', mode:'close_loop', patterns:[/\bnot interested\b/i, /\b(?:does not|doesn['’]?t|do not|don['’]?t)\s+want\s+to\s+(?:proceed|move forward)\b/i, /\b(?:will not|won['’]?t)\s+(?:proceed|move forward)\b/i, /\bnot moving forward\b/i, /\bdeclined\b/i] },
...
function textCue(text = '') { for (const cue of CUES) if (cue.patterns.some((pattern) => pattern.test(text))) return cue; return null; }
...
export function deriveRecoveryContext(input = {}) {
  const evidence = [], conflicts = [], text = String(input.lastContact ?? '');
  ...
  const contactState = input.contactPermission === 'allowed' ? 'allowed' : 'unknown';
  if (contactState === 'allowed') evidence.push('Structured contact permission: allowed.');
  else evidence.push('Contact permission is not explicitly confirmed.');
  ...
  if (explicitBlocker && stageBlocker && explicitBlocker !== stageBlocker) conflicts.push(`Selected objection (${explicitBlocker}) differs from the stage-implied blocker (${stageBlocker}); using the selected objection.`);
  if (explicitBlocker && cue?.blocker && explicitBlocker !== cue.blocker) conflicts.push(`Last-contact text suggests ${cue.blocker}, but the selected objection is ${explicitBlocker}; using the selected objection.`);
  ...
  if (cue?.kind === 'competitor_loss') {
    engagementState = 'lost'; recoveryMode = 'close_loop'; primaryBlocker = explicitBlocker ?? 'competitor';
  } else if (cue?.kind === 'declined') {
    engagementState = 'lost'; recoveryMode = 'close_loop';
