import { formatCurrencyAmount } from './domain.js';
...
export function scorePriority(input = {}, context = {}) {
  ...
  const quoteDetail=hasQuoteAmount?`$${formatCurrencyAmount(input.quoteAmount)}`:'Not supplied';
  if(hasQuoteAmount&&input.quoteAmount>=10000&&context.recoveryMode!=='close_loop'&&context.recoveryMode!=='reactivation'){score+=3;factors.push({label:'Quote value',delta:3,detail:quoteDetail});}else factors.push({label:'Quote value',delta:0,detail:quoteDetail});
